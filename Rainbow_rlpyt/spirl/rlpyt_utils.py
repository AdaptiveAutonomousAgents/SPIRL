...
class MinibatchRlEvalWandb(MinibatchRlEval):
    ...
    def __init__(self, final_eval_only=False, eval_itr0=True, save_ckpt_root=None,
                 *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.final_eval_only = final_eval_only
        self.eval_itr0 = eval_itr0
        self.save_ckpt_root = save_ckpt_root
        self.summary = {}

    def maybe_update_summary(self, key, value):
        if key not in self.summary.keys():
            self.summary[key] = value
        else:
            self.summary[key] = max(value, self.summary[key])

        logger.record_tabular(key, self.summary[key])

    def log_diagnostics(self, itr, eval_traj_infos, eval_time):
        cum_steps = (itr + 1) * self.sampler.batch_size * self.world_size
        logger.record_tabular('cum_steps', cum_steps)
            ...
            for k in traj_infos[0]:
                if not k.startswith("_"):
                    ...
                    if k == 'GameScore':
                        ...
                        logger.record_tabular(k + "Normalized", normalized_score)
                        logger.record_tabular(k + "DERNormalized", der_normalized_score)
                        logger.record_tabular(k + "NatureNormalized", nature_normalized_score)

                        self.maybe_update_summary(k+"Best", np.average(values))
                        self.maybe_update_summary(k+"NormalizedBest", normalized_score)
                        self.maybe_update_summary(k+"DERNormalizedBest", der_normalized_score)
                        self.maybe_update_summary(k+"NatureNormalizedBest", nature_normalized_score)
    ...
    def evaluate_agent(self, itr):
        ...
        """
        if not self.eval_itr0 and itr==0:
            logger.log("eval_itr=0, so the evaluation at the zero iteration will return null information")
            traj_infos = []
            eval_time = 0.0
            return traj_infos, eval_time

        if self.eval_itr0 and itr > 0:
            self.pbar.stop()
        ...
        if eval:
            ...
            eval_time = -time.time()
            if self.save_ckpt_root is not None:
                torch.save(self.agent.search.network.state_dict(),
                           os.path.join(self.save_ckpt_root, f'{itr}.pth'))
            eval_time += time.time()
            print(f'-------save ckpt, cost {eval_time/60} min------')
    ...
    def train(self):
        ...
        for itr in range(n_itr):
            ...
            with logger.prefix(f"itr #{itr} "):
                ...
                samples, traj_infos = self.sampler.obtain_samples(itr)  # interact with env and collect samples according to sampler.batch_B and samples.batch_T
...
class OneToOneSerialEvalCollector(SerialEvalCollector):
    def collect_evaluation(self, itr):
        # just replace the for loop in SerialEvalCollector by a while loop?
...
class SerialSampler(BaseSampler):
    ...
    def initialize(
        ...
        """
        B = self.batch_spec.B  # sampler.batch_B
