...
class SPRAgent(AtariCatDqnAgent):
    ...
    def eval_mode(self, itr):
        ...
        pre-training eval."""
        super().eval_mode(itr)  # here will set epsilon to self.eps_eval
    ...
    def sample_mode(self, itr):
        ...
        super().sample_mode(itr)  # here will update the epsilon
        self.search.epsilon = self.distribution.epsilon
...
class SPRActionSelection(torch.nn.Module):
    ...
    @torch.no_grad()
    def run(self, obs):
        # NOTE: here the obs comes from env, in range [0, 1]
        if self.network.image_obs:  # image obs shape: (f, c, h, w)
            env_obs_dim = 4
        elif self.network.ram_obs or self.network.ram_kp_pos_obs \
            or self.network.ram_kp_info_obs:  # e.g. ram obs shape: (f, 128)
            env_obs_dim = 2
        elif self.network.vit_rec_obs or self.network.vit_ram_obs:
            env_obs_dim = len(self.network.image_shape)
        else:
            raise NotImplementedError

        while len(obs.shape) <= env_obs_dim:  # unsqueeze one more dim besides the data dim, e.g. batch size
            obs.unsqueeze_(0)
        ...
        if self.first_call:
            # because of rlpyt.samplers.buffer.get_example_output
