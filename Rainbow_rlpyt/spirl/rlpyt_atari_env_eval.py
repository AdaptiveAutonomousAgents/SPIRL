...
class AtariEnvEval(Env):
    ...
    def __init__(self,
        ...
        if self.image_obs:
            ...
            self.grayscale = grayscale
            self.vit_loss_type = vit_loss_type
            ...
            self.imagesize = features_extractor_kwargs['img_size']
        ...
        if disturb is not None:
            assert (self.image_obs or self.vit_rec_obs)
        self.disturb = disturb
        self.num_pixel = num_pixel
        self.num_black_patch = num_black_patch
        if num_pixel is None:
            if self.disturb == "singlePixel0":
                self.num_pixel = int(210 * 160 * (num_pixel_ratio/100.0))
            elif self.disturb == "singlePixelResized":
                self.num_pixel = int(self.imagesize * self.imagesize * (num_pixel_ratio/100.0))
        if self.disturb == "fixWhite0":
            self.fix_noise_x = np.random.randint(210, size=self.num_pixel)
            self.fix_noise_y = np.random.randint(160, size=self.num_pixel)
            # self.fix_nosie_val = np.random.randint(255, size=self.num_pixel, dtype=np.uint8)
        if self.disturb == "fixWhiteResized":
            self.fix_noise_x = np.random.randint(self.imagesize, size=self.num_pixel)
            self.fix_noise_y = np.random.randint(self.imagesize, size=self.num_pixel)
            # self.fix_nosie_val = np.random.randint(255, size=self.num_pixel, dtype=np.uint8)
        if self.disturb == "fixBlackPatch":
            if num_black_patch == 0 or num_black_patch is None:
                self.disturb = None
            self.fix_patch_x = np.random.randint(12, size=self.num_black_patch)
            self.fix_patch_y = np.random.randint(12, size=self.num_black_patch)
        if self.disturb == "randBlackPatch":
            if num_black_patch == 0 or num_black_patch is None:
                self.disturb = None
        print(f'********disturb={self.disturb}, num_pixel={self.num_pixel}, num_black_patch={self.num_black_patch}')
    ...
    def add_frame_noise(self, frame):
        if self.disturb == "singlePixel0":
            if self.num_pixel > 0:
                rand_pos_x = np.random.randint(frame.shape[0], size=self.num_pixel)
                rand_pos_y = np.random.randint(frame.shape[1], size=self.num_pixel)
                # rand_pos = np.repeat(rand_pos, self.imagesize.shape[0], axis=0)
                rand_val = np.random.randint(255, size=self.num_pixel, dtype=np.uint8)
                # img = np.put_along_axis(img, indices=rand_pos, )
                for i in range(self.num_pixel):
                    # frame[rand_pos_x[i], rand_pos_y[i], :] = rand_val[i]
                    frame[rand_pos_x[i], rand_pos_y[i], 0] = rand_val[i]
        elif self.disturb == "singlePixelResized":
            pass
        elif self.disturb == "fixWhite0":
            for i in range(self.num_pixel):
                frame[self.fix_noise_x[i], self.fix_noise_y[i], :] = 255
        elif self.disturb in ["fixWhiteResized", "fixBlackPatch", "randBlackPatch"]:
            pass
        else:
            raise NotImplementedError
    
    def add_img_noise(self, img):
        if self.disturb == "singlePixel0":
            pass
        elif self.disturb == "singlePixelResized":
            if self.num_pixel > 0:
                rand_pos_x = np.random.randint(img.shape[0], size=self.num_pixel)
                rand_pos_y = np.random.randint(img.shape[1], size=self.num_pixel)
                # rand_pos = np.repeat(rand_pos, self.imagesize.shape[0], axis=0)
                rand_val = np.random.randint(255, size=self.num_pixel, dtype=np.uint8)
                # img = np.put_along_axis(img, indices=rand_pos, )
                for i in range(self.num_pixel):
                    # img[:, rand_pos_x[i], rand_pos_y[i]] = rand_val[i]
                    img[0, rand_pos_x[i], rand_pos_y[i]] = rand_val[i]
        elif self.disturb == "fixWhite0":
            pass
        elif self.disturb == "fixWhiteResized":
            for i in range(self.num_pixel):
                img[:, self.fix_noise_x[i], self.fix_noise_y[i]] = 255
        elif self.disturb =="fixBlackPatch":
            pdb.set_trace()
            patch_size = int(img.shape[1]/12)
            for i in range(self.num_black_patch):
                img[:, self.fix_patch_x[i]*patch_size:(self.fix_patch_x[i]+1)*patch_size, self.fix_patch_y[i]*patch_size:(self.fix_patch_y[i]+1)*patch_size] = 0
        elif self.disturb =="randBlackPatch":
            patch_size = int(img.shape[1]/12)
            fix_patch_x = np.random.randint(12, size=self.num_black_patch)
            fix_patch_y = np.random.randint(12, size=self.num_black_patch)
            for i in range(self.num_black_patch):
                img[:, fix_patch_x[i]*patch_size:(fix_patch_x[i]+1)*patch_size, fix_patch_y[i]*patch_size:(fix_patch_y[i]+1)*patch_size] = 0
        else:
            raise NotImplementedError
    ...
    def _update_obs(self, action):
        ...
        if self.image_obs:
            ...
            if self.disturb is not None:
                self.add_frame_noise(self._max_frame)
