...
def get_args_parser():
    ...
    parser.add_argument("--mae_pretrained_path", type=str, default=None,
    ...
    parser.add_argument('--compare_data_dir', type=str, help='same data testing Transporter and PermaKey')
    parser.add_argument('--img_save_path', type=str, help='path taht save Transporter & PermaKey visualization')
    parser.add_argument('--device', default='cpu', choices=['cuda', 'cpu'],
                        help='device to use for training / testing, for torch.device(args.device)')

    parser.add_argument('--disturb', default=None, type=str, choices=["fixWhiteResized", "singlePixelResized"])
    parser.add_argument('--num_pixel', default=None, type=int)
    parser.add_argument('--loss_type', default='L2', type=str, choices=['L2', 'L1', 'L2trim1',
...
@torch.no_grad()
def evaluate(model, device, image_size, args):
    ...
    # Load files
    data_dir_test = os.path.join(args.compare_data_dir, "test")
    filenames_list = []
    for subdir, dirs, files in os.walk(data_dir_test):
        for file in files:  # opens each .npz file
            filenames_list.append(os.path.join(subdir, file))

    if args.disturb == "fixWhiteResized":
        data = np.load(filenames_list[0])
        obs = data['frames']
        fix_noise_x = np.random.randint(obs.shape[-2], size=args.num_pixel)
        fix_noise_y = np.random.randint(obs.shape[-2], size=args.num_pixel)
    img_cnt = 0
    for file in filenames_list:
        ...
        if obs.shape[-1] in [1, 3]:
            obs = np.einsum('bhwc->bchw', obs)
        for i in range(B):
            if i%8:
                continue
            # eval with each patches
            img_ls = torch.from_numpy(obs[i][None].astype('float32'))
            ...
            with torch.cuda.amp.autocast():
                ...
                if args.disturb == "fixWhiteResized":
                    for i in range(args.num_pixel):
                        samples[0][0, :, fix_noise_x[i], fix_noise_y[i]] = 1
                elif args.disturb == "singlePixelResized":
                    noise_x = np.random.randint(obs.shape[-2], size=args.num_pixel)
                    noise_y = np.random.randint(obs.shape[-2], size=args.num_pixel)
                    noise_val = np.random.uniform(0, 1, args.num_pixel)
                    for i in range(args.num_pixel):
                        samples[0][0, :, noise_x[i], noise_y[i]] = noise_val[i]
                # reconstruct with only cls token
                loss_mask_all, pred_mask_all, mask = model(
                    (samples[0], None), mask_ratio=1, loss_all=True, loss_type=args.loss_type)
                ...
                for idx in range(model.patch_embed.num_patches):
                    ...
                    surr_loss_all, surr_loss_idx, surr_pred, _ = model(
                        (samples[0], None), surr_mask_idx=idx, loss_all=True, loss_type=args.loss_type)
            ...
            img_save_dir = args.img_save_path

            os.makedirs(os.path.join(img_save_dir, f'{img_cnt:3d}'), exist_ok=True)
            # original imaeg
            ...
            plt.margins(0.0)
            plt.savefig(os.path.join(img_save_dir, f'{img_cnt:3d}', 'our_original.png'),
            ...
            plt.savefig(os.path.join(img_save_dir, f'{img_cnt:3d}', 'our_predicted.png'),
                        bbox_inches='tight', pad_inches=0)
            ...
            plt.savefig(os.path.join(img_save_dir, f'{img_cnt:3d}', 'our_error_map.png'),
                        bbox_inches='tight', pad_inches=0)
            ...
            for scale in [1,2]:
                ...
                plt.savefig(os.path.join(img_save_dir, f'{img_cnt:3d}', f'our_cumsum-{scale}.png'),
                            bbox_inches='tight', pad_inches=0)
                ...
                plt.savefig(os.path.join(img_save_dir, f'{img_cnt:3d}', f'our_dynamic45_{len_keep}-{scale}.png'),
                            bbox_inches='tight', pad_inches=0)
                ...
                plt.yticks(size=15,weight='bold')
                plt.xticks(size=15,weight='bold')
                plt.savefig(os.path.join(img_save_dir, f'{img_cnt:3d}', f'our_rev_cumsum_144-{scale}.png'),
                            bbox_inches='tight', pad_inches=0)

                # reverse cumsum and key patches selected by thres45
                plt.figure(figsize=(6,6))
                ax = plt.gca()
                ax_line_plot(cumval, ax, lw=2.5, norm_x=True)
                # ax_line_plot(cumval_diff, axes[plot_cnt%num_column],
                #             title=f'cumsum L1 normed & diff-seg-{seg}-scale-{scale}', marker='.', is_twin=True, color='r')
                ax.axvline(x=len_keep/vals.shape[-1], color='r', linestyle='dashed', lw=2.5)
                plt.yticks(size=15,weight='bold')
                plt.xticks(size=15,weight='bold')
                plt.savefig(os.path.join(img_save_dir, f'{img_cnt:3d}', f'our_rev_cumsum_1-{scale}.png'),
                            bbox_inches='tight', pad_inches=0)
            ...
            for mask_ratio in [0.90, 0.85, 0.80, 0.75, 0.7, 0.65, 0.6, 0.55, 0.5]:
                ...
                draw_mask(keep_id=keep_id, model=model, mask_shape=surr_loss_idx_mat.shape,
                        ori_img=(ori_img * 255).astype(int), ax=ax)
                plt.savefig(os.path.join(img_save_dir, f'{img_cnt:3d}',
                                        f'our_mask_ratio_{mask_ratio}_len_keep_{len_keep}.png'),
                        bbox_inches='tight', pad_inches=0)
            ...
            plt.savefig(os.path.join(img_save_dir, f'{img_cnt:3d}',
                                    f'our_pred_decoder_mask_and_pos.png'),
                        bbox_inches='tight', pad_inches=0)
            ...
            ax_plot(image=recon_mask_all, ax=ax)
            plt.savefig(os.path.join(img_save_dir, f'{img_cnt:3d}',
                                    f'our_pred_decoder_no_mask.png'),
                        bbox_inches='tight', pad_inches=0)

            img_cnt += 1
            if img_cnt % 10:
                print(f'img_cnt:{img_cnt}')
            if img_cnt > args.num_eval_data:
                break
...
def main(args):
    ...
    evaluate(model, device, image_size=ckpt['args'].image_size, args=args)
